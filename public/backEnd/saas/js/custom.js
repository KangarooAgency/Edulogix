            ...
            success: function(data) {
                $.each(data, function(i, item) {
                    if (item.length) {
                        ...
                        $("#select_student_div .current").html('select_student' +" *");
                        $("#select_student").find("option").not(":first").remove();
                        $("#select_student_div ul").find("li").not(":first").remove();
                    }
                });
...
$(document).ready(function() {
    $("#btn-assign-fees-group").click(function() {
        ...
        if (abc != "") {
            ...
            setTimeout(function() {
                toastr.error("Student not selected", "Error Alert", { timeOut: 5000 });
            }, 500);
        }
    });
});
    ...
    $("#addNewSubject").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                var subject_teacher = "";
                subject_teacher +=
                    "<div class='col-lg-12 mb-30' id='assign-subject-" + divCount + "'>";
                subject_teacher += "<div class='row'>";
                subject_teacher += "<div class='col-lg-5 mt-30-md'>";
                subject_teacher +=
                    "<select class='w-100 bb niceSelect form-control' name='subjects[]' style='display:none'>";
                subject_teacher +=
                ...
                subject_teacher +=
                    "<li data-value='' data-display='Select Subject' class='option selected'>Select Subject</li>";
                $.each(data[0], function(key, subject) {
                    subject_teacher +=
                        "<li data-value=" +
                        subject.id +
    ...
    $("#addNewRoom").click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                appendRow +=
                    "<select class='w-100 bb niceSelect class_room room_input'  name='room[]' id='room_" +
                    rowCount +
                    "'>";
                appendRow +=
                ...
                appendRow += "</select>";
                appendRow +=
                    "<span id='room_error-" + rowCount + "' class='text-danger'></span>";
                appendRow += "</div>";
                appendRow += "</div>";
                appendRow += "</div>";
                appendRow += "</td>";
...
$(document).on("click", "#create-option", function(event) {
    ...
    for (var i = 1; i <= number_of_option; i++) {
        ...
        appendRow += "<div class='input-effect'>";
        appendRow +=
            "<input class='primary-input form-control has-content' placeholder='option " +
            i +
...
$(document).on("click", ".fees-groups-print", function(event) {
    ...
    if (sList != "") {
        ...
        console.log("Fees group not selected");
            ...
            success: function(data) {
                ...
                $.each(data, function(i, value) {
                    ...
                    appendRow +=
                        ...
                        "]' value='" +
                        value.id +
                        "'><input class='primary-input form-control type='text' name='LU[" +
                        value.id +
                        "]' value='" +
                        value[lu] +
                        "'>";
...
$("form#assign_subject").on("submit", function() {
    ...
    if (abc != "") {
        setTimeout(function() {
            toastr.error(
                "May duplicate entry uccured, please check and try again!",
            ...
            $.ajax({
                ...
                success: function(data) {
                    setTimeout(function() {
                        toastr.success(
                            "Operation Success!",
                            "Success Alert", { iconClass: "toast-success" }, { timeOut: 2000 }
                        );
                    }, 500);
                },
                error: function(data) {
...
(function() {
    $(document).ready(function() {
        $(".switch-input-institution-approve").on("change", function() {
            if ($(this).is(":checked")) {
                var status = "on";
            } else {
                var status = "off";
...
$(document).ready(function() {
    $("#select-institution").change(function() {
        ...
        $.ajax({
            ...
            data: formData,
            dataType: "json",
            url: url + "/" + "administrator/ajax-get-class",
            success: function(data) {
                console.log(data);
...
// student Attendance
$(document).ready(function() {
    $("#select-institution-result").change(function() {
        var url = $("#url").val();
        ...
        $.ajax({
            ...
            data: formData,
            dataType: "json",
            url: url + "/" + "administrator/ajax-get-class-exam",
            success: function(data) {
                if (data.classes.length) {
                    $("#select_class").find("option").not(":first").remove();
                    $("#select_class_div ul").find("li").not(":first").remove();

                    $.each(data.classes, function(i, classA) {
                        $("#select_class").append(
                            $("<option>", {
                                value: classA.id,
                                text: classA.class_name,
                ...
                if (data.exams.length) {
                    $("#select_exam").find("option").not(":first").remove();
                    $("#select_exam_div ul").find("li").not(":first").remove();
                    ...
                    $("#select_exam_div .current").html("SELECT exam *");
                    $("#select_exam").find("option").not(":first").remove();
                    $("#select_exam_div ul").find("li").not(":first").remove();
                }
            },
